...
VkCommandPoolCreateInfo vkinit::command_pool_create_info(uint32_t qeueFamilyIndex, VkCommandPoolCreateFlags flags)
{
	VkCommandPoolCreateInfo info{};
	info.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
	info.pNext = nullptr;
	info.queueFamilyIndex = qeueFamilyIndex;
	info.flags = flags;
	return info;
}
...
VkImageCreateInfo vkinit::image_create_info(VkFormat format, VkImageUsageFlags usageFlags, VkExtent3D extent)
{
	...
	// for MSAA. We will not be uding it by default, so default it to 1 sample per pixel.
